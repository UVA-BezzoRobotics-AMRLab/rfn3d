...
// ROS2-only helpers: convert the core's ROS-free trajectory to messages and
// visualize it. The trajectory is already sampled by the core, so this maps
// each state to a message point (and copies out the jerks the wrapper needs to
// seed the next replan's initial state).
namespace utils
{

    inline trajectory_msgs::msg::MultiDOFJointTrajectory
    ...
    inline void visualize_traj(const trajectory_msgs::msg::MultiDOFJointTrajectory &trajectory,
                               const rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr &trajVizPub,
                               const std::string &frame_str,
                               const rclcpp::Time &stamp)
    {
        ...
        {
            const trajectory_msgs::msg::MultiDOFJointTrajectoryPoint &startPoint = trajectory.points[i];
            const trajectory_msgs::msg::MultiDOFJointTrajectoryPoint &endPoint = trajectory.points[i + 1];
