...
Planner::Planner(ros::NodeHandle &nh) {
  ...
  nh.param<std::string>("topic_goal", _topic_goal, "/goal");
  ...
  ROS_INFO("topic goal: %s", _topic_goal.c_str());
  ...
  // Subscribers
  _goal_sub = nh.subscribe("/goal", 1, &Planner::goal_cb, this);
  ...
  _visualizer = std::make_unique<Visualizer>(nh, _frame_id);
}
...
void Planner::goal_cb(const geometry_msgs::PoseStamped::ConstPtr &msg) {
  _goal = Eigen::Vector3d(msg->pose.position.x, msg->pose.position.y, 1.5);
  _is_goal_set = true;
}
...
void Planner::get_ptcld_from_octree(const Eigen::Vector3d &origin,
  ...
  ROS_INFO("Found %zu points in octree", pcl_eigen.size());
...
bool Planner::plan(bool is_failsafe) {
  ...
  **************************************/
  ...
  ROS_INFO("found polys: %zu", _hpolys.size());
  ROS_INFO_STREAM("first poly:\n" << _hpolys[0]);
  _visualizer->visualizePolytope(_hpolys);
