...
// std::unique_ptr<JPSPlanner3D> planner_ptr(new JPSPlanner3D(true)); // Declare
// a planner
...
Planner::Planner(ros::NodeHandle &nh) {
  ...
  nh.param<std::string>("topic_odom", _topic_odom,
  ...
  nh.param<std::string>("topic_traj", _topic_traj,
                        "/firefly/command/trajectory");
  nh.param<std::string>("topic_traj_ref_viz", _topic_traj_ref_viz,
                        "/traj_ref_viz");

  // service params
  nh.param<std::string>("service_mrs_traj", _service_mrs_traj,
  ...
  // planner_ptr = std::make_unique<JPSPlanner3D>(false); // Assuming you want
  // verbose mode
  ...
  // Publishers
  _jps_pub = nh.advertise<nav_msgs::Path>("/jps_path", 1);
  ...
  _traj_viz_pub =
      nh.advertise<visualization_msgs::MarkerArray>(_topic_traj_viz, 1);
  _traj_pub =
      nh.advertise<trajectory_msgs::MultiDOFJointTrajectory>(_topic_traj, 1);

  // Services
  _mrs_traj_client =
      nh.serviceClient<mrs_msgs::TrajectoryReferenceSrv>(_service_mrs_traj);
...
void Planner::odom_cb(const nav_msgs::Odometry::ConstPtr &msg) {
  _odom = Eigen::Vector3d(msg->pose.pose.position.x, msg->pose.pose.position.y,
  ...
  if (_sent_traj.points.size() > 0) {
    ...
    int traj_ind =
        std::min((int)(t / _traj_dt), (int)_sent_traj.points.size() - 1);
...
void Planner::get_array_from_octree(const Vec3f &origin, const Vec3i &size,
  ...
  for (int i = 0; i < size[0]; i++) {
    ...
    for (int j = 0; j < size[1]; j++) {
      ...
      for (int k = 0; k < size[2]; k++) {
        ...
        octomap::OcTreeNode *node = _octree->search(
            test_point); // Retrieve the OcTreeNode for the current point
...
void Planner::get_ptcld_from_octree(const Eigen::Vector3d &origin,
  ...
  octomap::point3d min =
      octomap::point3d(origin[0] - size[0] / 2, origin[1] - size[1] / 2,
                       origin[2] - size[2] / 2);
  octomap::point3d max =
      octomap::point3d(origin[0] + size[0] / 2, origin[1] + size[1] / 2,
                       origin[2] + size[2] / 2);
  ...
  for (octomap::OcTree::leaf_bbx_iterator
           it = _octree->begin_leafs_bbx(min, max),
           end = _octree->end_leafs_bbx();
       it != end; ++it) {
...
void Planner::map_cb(const octomap_msgs::Octomap::ConstPtr &msg) {
  ...
  _octree = std::shared_ptr<octomap::OcTree>(
...
bool Planner::plan(bool is_failsafe) {
  ...
  **************************************/
  ...
  if (_sent_traj.points.size() == 0) {
    ...
    if (is_failsafe) {
      ...
      initialPVAJ.col(1) =
          Eigen::Vector3d(p.velocities[0].linear.x, p.velocities[0].linear.y,
                          p.velocities[0].linear.z);
      initialPVAJ.col(2) = Eigen::Vector3d(p.accelerations[0].linear.x,
                                           p.accelerations[0].linear.y,
                                           p.accelerations[0].linear.z);
  ...
  **************************************/
  ...
  std::shared_ptr<JPS::VoxelMapUtil> map_util =
      std::make_shared<JPS::VoxelMapUtil>();
  JPSPlanner3D jps_planner(false);

  map_util->setMap(origin, dim, occ_array, res);

  jps_planner.setMapUtil(map_util); // Set collision checking function
  jps_planner.updateMap();

  bool valid_jps = jps_planner.plan(start, goal, 1,
                                    true); // Plan from start to goal using JPS

  double dist = total_distance3f(jps_planner.getRawPath());
  printf("JPS Path Distance: %.2f\n", dist);

  if (dist < 1e-2) {
    ROS_WARN("JPS distance zero");
    return false;
  }

  auto path_jps = jps_planner.getRawPath();
  ...
  if (path_jps.size() < 1) {
    ROS_ERROR("JPS failed");
    return false;
  }

  // visualize JPS
  nav_msgs::Path jps_path;
  jps_path.header.frame_id = _frame_id;
  ...
  for (const auto &jps_pt : path_jps) {
    ...
    geometry_msgs::PoseStamped pose;
    pose.pose.position.x = jps_wpt[0];
    pose.pose.position.y = jps_wpt[1];
    pose.pose.position.z = jps_wpt[2];
    pose.pose.orientation.w = 1.0;
    jps_path.poses.push_back(pose);
  }

  _jps_pub.publish(jps_path);
  ...
  initialState.setAccel(initialPVAJ(0, 2), initialPVAJ(1, 2),
                        initialPVAJ(2, 2));
  ...
  **************************************/
  ...
  if (_sent_traj.points.size() != 0) {
    ...
    int start_ind =
        std::min((int)(t1 / _traj_dt), (int)_sent_traj.points.size() - 1) + 1;
    int traj_ind =
        std::min((int)(t2 / _traj_dt), (int)_sent_traj.points.size() - 1);

    trajectory_msgs::MultiDOFJointTrajectory a_traj, b_traj;
    b_traj = utils::convert_traj_to_msg(_traj_solver.X_temp_, _traj_dt,
                                        _frame_id, _jerks);

    for (int i = start_ind; i < traj_ind; ++i) {
      a_traj.points.push_back(_sent_traj.points[i]);
      a_traj.points.back().time_from_start =
          ros::Duration((i - start_ind) * _traj_dt);
    }
    ...
    for (int i = 0; i < b_traj.points.size(); ++i) {
      ...
      a_traj.points.back().time_from_start =
          ros::Duration(start_time + i * _traj_dt);
    }
    ...
    _sent_traj = utils::convert_traj_to_msg(_traj_solver.X_temp_, _traj_dt,
                                            _frame_id, _jerks);
  }
  ...
  mrs_msgs::TrajectoryReferenceSrv srv_trajectory_reference =
      utils::convert_traj_to_mrs_srv(_sent_traj, _frame_id);
