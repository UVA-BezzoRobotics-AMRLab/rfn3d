...
RRTPlanner::RRTPlanner(ros::NodeHandle &nh)
{
    ...
    esdf_server_ = std::make_shared<voxblox::EsdfServer>(nh, nh_private);


    // get xyz pointcloud from esdf_server_
    voxblox::Point lowerb, upperb;
    Eigen::Vector3d lower_bound = Eigen::Vector3d(-10, -10, -10);
    Eigen::Vector3d upper_bound = Eigen::Vector3d(10, 10, 10);
    lowerb << lower_bound.x(), lower_bound.y(), lower_bound.z();
    upperb << upper_bound.x(), upper_bound.y(), upper_bound.z();

    voxblox::EsdfMap::Ptr esdf_map = esdf_server_->getEsdfMapPtr();
    
    // Get the voxel size (assuming a uniform voxel size)
    const voxblox::FloatingPoint voxel_size = esdf_map->block_size();
